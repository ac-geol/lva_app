...
tables only. The 88 tests are self-contained, so `git clone` + `pytest` proves
...
| branch | what it is |
|---|---|
| `main` | Everything. The engine, 88 tests, downhole compositing (off by default), and every result written up in `docs/METHOD_COMPARISON.md`. |
